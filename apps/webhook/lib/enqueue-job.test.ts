...
import { createJobDispatcher, enqueueJob, resetJobDispatcherCache } from "./enqueue-job.ts";
import type { TasksClientLike } from "./cloud-tasks.ts";

afterEach(() => {
  resetJobDispatcherCache();
});
...
test("環境が揃えば Cloud Tasks を使う", async () => {
  ...
  const dispatcher = createJobDispatcher(tasksEnv, { tasksClient: client });
  await dispatcher(JSON.stringify({ event_id: "Ev2" }));
...
test("mise の WORKER_URL だけでは HTTP にする", async () => {
  ...
  const enqueue = createJobDispatcher(
...
test("ローカルで WORKER_URL が無いと落とす", () => {
  expect(() => createJobDispatcher({})).toThrow("WORKER_URL is required");
});

test("enqueueJob は dispatcher を使い回す", async () => {
  const fetchFn = vi.fn<typeof fetch>(async () => new Response("{}", { status: 200 }));
